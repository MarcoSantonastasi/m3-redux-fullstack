...
export default class ShowAssets extends Component {
  ...
  render() {
    return (
      <div>
        <p>
          {" "}
          {window.CS.getUIState().waitingForResponse.toString()}
          {window.CS.getUIState().counter}
        </p>
        <h1>simple asset management application</h1>
        <p>
          to create a new asset click this button:&nbsp;
  ...
  handleCreateAsset() {
    ...
    axios
      .post("http://localhost:8080/assets/add/", newAsset)
      .then(res => console.log(res.data))
      .catch(error =>
        console.log("Error: ", error, "while saving asset", newAsset)
      );
